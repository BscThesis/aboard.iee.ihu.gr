      ...
      <div class="columns">
        <div class="column is-two-thirds">
              ...
              <div class="field has-addons">
                <div class="control is-expanded">
                  <input
                    ...
                    v-model="search"
                  />
                  <p class="help is-gray is-italic">Προσθέστε όρους και πατήστε Αναζήτηση</p>
                </div>
                <div class="control">
                  <a class="button" @click="doSearch()" :disabled="searchInput.length == 0">
                    <span class="icon">
          ...
          <div class="field is-grouped is-grouped-multiline">
            <div class="control" v-for="(item, index) in searchInput" v-bind:key="index">
              <div class="tags">
                <span class="tag is-rounded is-light">
                  <span>{{ item }}</span>
                  <button class="delete is-small" v-on:click="removeFromSearchList(index)"></button>
                </span>
              </div>
            </div>
          </div>
        ...
        <div class="column is-one-third">
          <table class="table is-narrow is-hoverable is-fullwidth">
            ...
            <tbody>
              <tr v-for="pin in pinned" v-if="pinned.length > 0" v-bind:key="pin.id">
                <td class="has-text-centered">
                  <a v-bind:href="'/announcements/' + pin.id">{{ pin.title }}</a>
                </td>
              </tr>
              <tr v-if="pinned.length == 0">
                <td class="has-text-centered is-unselectable">Καμία ανακοίνωση</td>
              </tr>
            </tbody>
          </table>
        </div>
